    ...
    if selected_episode:
        ...
        if not trade_data.empty:
            ...
            for i, (idx, row) in enumerate(trade_window.iterrows()):
                ...
                if previous_price is not None and action in ['sell', 'open short', 'close_short']:
                    price_increased = curr_price > previous_price
                    ...
                    if price_increased and reward_value > 0 and action == 'open short':
                        print(f"Warning: Inconsistent reward for short position at {row['datetime']}: "
                              f"Price increased ({previous_price:.2f} to {curr_price:.2f}) but reward is positive ({reward_value:.4f})")
