...
class PPOTrader:
    def __init__(self, env, hidden_size=512, policy_lr=1e-4, gamma=0.99, 
        ...
        self.policy_scheduler = torch.optim.lr_scheduler.ReduceLROnPlateau(
            self.policy_optimizer, mode='max', factor=0.5, patience=5, verbose=True
        )
...
def compute_donchian_channels(df, window=20):
    """Compute Donchian channels (highest high and lowest low over a period)"""
    # Check if high/low columns exist, if not, try to use other columns or create them
    if 'high' not in df.columns:
        if 'High' in df.columns:  # Try capitalized version
            df['high'] = df['High']
        elif 'h' in df.columns:  # Try abbreviated version
            df['high'] = df['h']
        elif 'close' in df.columns:  # Fallback to close price
            print("Warning: 'high' column not found, using 'close' as substitute")
            df['high'] = df['close']
        else:
            raise ValueError("Cannot compute Donchian channels: no 'high' or equivalent column found")
    
    if 'low' not in df.columns:
        if 'Low' in df.columns:  # Try capitalized version
            df['low'] = df['Low']
        elif 'l' in df.columns:  # Try abbreviated version
            df['low'] = df['l']
        elif 'close' in df.columns:  # Fallback to close price
            print("Warning: 'low' column not found, using 'close' as substitute")
            df['low'] = df['close']
        else:
            raise ValueError("Cannot compute Donchian channels: no 'low' or equivalent column found")
...
def preprocess_data(file_path):
    """Preprocess data for the trading environment"""
    print(f"Loading data from {file_path}")
    df = pd.read_csv(file_path)
    
    # Print column names for debugging
    print(f"Columns in data file: {df.columns.tolist()}")
    ...
    for col in required_columns:
        if col not in df.columns:
            ...
            if col == 'close' and 'Close' in df.columns:
                ...
                df['datetime'] = df['date']
            elif col == 'datetime' and 'time' in df.columns:
                df['datetime'] = df['time']
            else:
    ...
    if 'datetime' in df.columns:
        if pd.api.types.is_numeric_dtype(df['datetime']):
            # If datetime is numeric (timestamp), convert to datetime
            ...
            try:
                ...
                print("Warning: Could not parse datetime column, using index instead")
    ...
    # Ensure OHLC columns exist (create from close if needed)
    if 'open' not in df.columns:
        if 'Open' in df.columns:
            ...
            print("Warning: 'open' column not found, using 'close' as substitute")
            df['open'] = df['close']
    ...
    for col in features_to_normalize:
        if col in df.columns:
            ...
            df[f'{col}_norm'] = df[f'{col}_norm'].fillna(0)  # Changed from method='ffill' to just 0
...
class ProTraderEnv(Env):
    ...
    def step(self, action):
        ...
        if action == 0:  # Hold
            ...
            if self.position:
                ...
                if profit > 0:
                    ...
                    reward = -base_penalty - consecutive_penalty
        ...
        reward = networth_change / self.initial_balance
        ...
        if self.position and self.trade_duration > 20:  # Penalize after 20 steps
            reward -= 0.001 * (self.trade_duration - 20)  # Small incremental penalty
