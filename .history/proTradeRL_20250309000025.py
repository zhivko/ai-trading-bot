...
class ProTraderEnv(Env):
    def __init__(self, df, initial_balance=10000, debug=False):
        ...
        # More balanced reward parameters
        self.positive_trade_multiplier = 1.5  # Reduced from 2.0
        self.negative_trade_multiplier = 1.5  # Reduced from 2.5 and made equal for both directions
        self.consecutive_loss_penalty = 0.1   # Reduced from 0.2
        
        # Track consecutive losses separately for long and short
    ...
    def reset(self, seed=None, options=None):
        ...
        # Reset consecutive losses counter for both directions
        self.consecutive_long_losses = 0
        self.consecutive_short_losses = 0
    ...
    def step(self, action):
        ...
        if action == 0:  # Hold
            ...
            if self.position:
                ...
                # More balanced penalties/rewards for holding positions
                if self.trade_unrealized_profit < 0:
                    # Reduced penalty for holding a losing position
                    unrealized_loss_penalty = abs(self.trade_unrealized_profit) / self.initial_balance * 0.2
                    reward -= unrealized_loss_penalty
                    
                    # Smaller duration penalty
                    duration_penalty = 0.0005 * self.trade_duration
                    reward -= duration_penalty
                else:
                    # Increased reward for holding a winning position
                    unrealized_profit_reward = self.trade_unrealized_profit / self.initial_balance * 0.2
                    reward += unrealized_profit_reward
        
        elif action == 1:  # Close position
            action_taken = "CLOSE"
            if self.position:
                position_type = self.position_type  # Save for tracking
                ...
                if profit > 0:
                    ...
                    if self.trade_duration < 10:
                        reward += 0.05 * reward  # Reduced from 0.1
                    
                    # Reset consecutive losses for the specific direction
                    ...
                    # Add consecutive loss penalty for the specific direction
                    if position_type == 'long':
                        consecutive_penalty = self.consecutive_loss_penalty * self.consecutive_long_losses
                        self.consecutive_long_losses += 1
                    else:
                        consecutive_penalty = self.consecutive_loss_penalty * self.consecutive_short_losses
                        self.consecutive_short_losses += 1
                        
                    # Add duration penalty for long losing trades (reduced)
                    duration_penalty = 0.005 * min(self.trade_duration / 10, 1.0)
            ...
            if not self.position:
                ...
                self.trade_duration = 0
                # Equal penalty for opening any position
                reward = -0.001
        ...
        if self.current_step >= self.max_steps - 1:
            ...
            if self.position:
                ...
                if final_profit < 0:
                    # Reduced end-of-episode penalty
                    reward = -abs(final_profit) / self.initial_balance * self.negative_trade_multiplier * 0.5
        ...
        reward_scale = 50.0  # Reduced from 100.0
        networth_reward = networth_change / self.initial_balance * reward_scale
        
        # Combine rewards with more balance
        if reward == 0:  # If no specific action reward was set
            reward = networth_reward
        else:
            # Add networth change component with equal weight
            reward = (reward + networth_reward) / 2
        ...
        # Scale networth change reward/penalty (reduced multipliers)
        if step_networth_change > 0:
            reward += step_networth_change * self.positive_trade_multiplier * 0.5
        else:
            reward += step_networth_change * self.negative_trade_multiplier * 0.5
        
        # Reduced penalty for holding a position too long without profit
        if self.position and self.trade_duration > 20 and self.trade_unrealized_profit <= 0:
            reward -= 0.001 * (self.trade_duration - 20)  # Reduced from 0.002
