...
class ProTraderEnv(Env):
    def __init__(self, df, initial_balance=10000, debug=False):
        ...
        self.reward_multiplier = 1.0  # Base multiplier for all rewards
        
        # Trade tracking
        self.trade_start_price = None
        self.trade_start_step = None
        self.trade_predicted_profitable = None  # Prediction of trade profitability
    ...
    def reset(self, seed=None, options=None):
        ...
        self.trade_start_price = None
        self.trade_start_step = None
        self.trade_predicted_profitable = None
    ...
    def _predict_trade_profitability(self, position_type, price):
        """Predict if a trade will be profitable based on technical indicators"""
        # Get current step indicators
        current_step = self.current_step
        
        # Default prediction (neutral)
        prediction = 0
        
        # Check technical indicators if available
        if 'rsi_norm' in self.df.columns and current_step < len(self.df):
            rsi = self.df['rsi_norm'].iloc[current_step]
            # For long positions: RSI < -1 is oversold (bullish), RSI > 1 is overbought (bearish)
            # For short positions: RSI > 1 is overbought (bullish for shorts), RSI < -1 is oversold (bearish for shorts)
            if position_type == 'long' and rsi < -0.5:  # Oversold, good for long
                prediction += 0.5
            elif position_type == 'long' and rsi > 0.5:  # Overbought, bad for long
                prediction -= 0.5
            elif position_type == 'short' and rsi > 0.5:  # Overbought, good for short
                prediction += 0.5
            elif position_type == 'short' and rsi < -0.5:  # Oversold, bad for short
                prediction -= 0.5
        
        # Check trend indicators if available
        if 'ema_short_norm' in self.df.columns and 'ema_long_norm' in self.df.columns and current_step < len(self.df):
            ema_short = self.df['ema_short_norm'].iloc[current_step]
            ema_long = self.df['ema_long_norm'].iloc[current_step]
            # Uptrend: short EMA > long EMA, good for long
            # Downtrend: short EMA < long EMA, good for short
            if position_type == 'long' and ema_short > ema_long:
                prediction += 0.5
            elif position_type == 'long' and ema_short < ema_long:
                prediction -= 0.5
            elif position_type == 'short' and ema_short < ema_long:
                prediction += 0.5
            elif position_type == 'short' and ema_short > ema_long:
                prediction -= 0.5
    ...
    def _open_position(self, price, position_type):
        ...
        # Predict if this trade will be profitable
        self.trade_predicted_profitable = self._predict_trade_profitability(position_type, price)
    ...
    def step(self, action):
        ...
        unrealized_profit = 0
        if self.position:
            unrealized_profit = self._calculate_unrealized_profit(current_price)
        ...
        if action == 0:  # Hold
            ...
            if self.position:
                ...
                # Reward/penalty based on unrealized profit and prediction
                if unrealized_profit > 0:
                    # Reward for holding a profitable position
                    reward = 0.01 * self.reward_multiplier
                    
                    # Extra reward if the prediction was correct
                    if self.trade_predicted_profitable > 0:
                        reward += 0.005 * self.trade_predicted_profitable
                else:
                    # Penalty for holding a losing position
                    reward = -0.01 * self.reward_multiplier
                    
                    # Reduced penalty if the prediction was negative (we expected this)
                    if self.trade_predicted_profitable < 0:
                        reward *= 0.5
            ...
            if self.position:
                ...
                profit = self._calculate_unrealized_profit(current_price)
                
                # Close the position
                self._close_position(current_price)
                
                # Reward/penalty based on overall trade profit
                if profit > 0:
                    # Reward for profitable trade
                    reward = 0.1 * profit / self.initial_balance * self.reward_multiplier
                    
                    # Extra reward if the prediction was correct
                    if self.trade_predicted_profitable > 0:
                        reward *= (1 + 0.2 * self.trade_predicted_profitable)
                else:
                    # Penalty for losing trade
                    reward = -0.1 * abs(profit) / self.initial_balance * self.reward_multiplier
            ...
            if not self.position:
                ...
                # Initial reward/penalty based on prediction
                if self.trade_predicted_profitable > 0:
                    # Small reward for opening a potentially profitable position
                    reward = 0.005 * self.trade_predicted_profitable * self.reward_multiplier
                else:
                    # Small penalty for opening a potentially unprofitable position
                    reward = 0.005 * self.trade_predicted_profitable * self.reward_multiplier
        ...
        if self.current_step >= self.max_steps - 1:
            ...
            if self.position:
                final_profit = self._calculate_unrealized_profit(current_price)
                self._close_position(current_price)
                
                # Final reward/penalty
                if final_profit > 0:
                    reward = 0.05 * final_profit / self.initial_balance * self.reward_multiplier
                else:
                    reward = -0.05 * abs(final_profit) / self.initial_balance * self.reward_multiplier
            
            return self._next_observation(), reward, True, False, {}
        
        # Calculate net worth change
        current_networth = self.get_networth()
        networth_change = current_networth - prev_networth
        
        # Add networth change component to reward
        networth_reward = 0.5 * networth_change / self.initial_balance * self.reward_multiplier
        reward += networth_reward
        
        # Penalty for holding a position too long
        if self.position and self.trade_duration > 20 and unrealized_profit <= 0:
            reward -= 0.001 * (self.trade_duration - 20)
