...
class ProTraderEnv(Env):
    def __init__(self, df, initial_balance=10000, debug=False):
        ...
        # Simple balanced reward parameters
        self.trade_open_penalty = 0.001  # Small fixed penalty for opening any position
        self.profit_reward_factor = 1.0  # Reward factor for profitable trades
        self.loss_penalty_factor = 1.0   # Penalty factor for losing trades
        
        # Track trade performance
        self.entry_networth = None       # Net worth at trade entry
    ...
    def step(self, action):
        ...
        if action == 0:  # Hold
            ...
            if self.position:
                ...
                # Reward or penalize based on unrealized profit/loss
                if unrealized_pnl > 0:
                    # Small reward for holding a profitable position
                    reward = 0.01 * self.profit_reward_factor
                else:
                    # Small penalty for holding a losing position
                    reward = -0.01 * self.loss_penalty_factor
            ...
            if self.position:
                ...
                # Calculate net worth change from entry to exit
                current_networth = self.get_networth()
                if self.entry_networth is not None:
                    networth_change = current_networth - self.entry_networth
                    
                    # Reward or penalize based on overall trade performance
                    if networth_change > 0:
                        # Reward for profitable trade
                        reward = 0.1 * networth_change / self.initial_balance * self.profit_reward_factor
                    else:
                        # Penalty for losing trade
                        reward = -0.1 * abs(networth_change) / self.initial_balance * self.loss_penalty_factor
                
                # Reset entry net worth
                self.entry_networth = None
        
        elif action == 2:  # Buy (long)
            action_taken = "BUY"
            if not self.position:
                # Store entry net worth
                self.entry_networth = self.get_networth()
                
                # Open long position
                self._open_position(current_price, 'long')
                self.trade_duration = 0
                
                # Small fixed penalty for opening a position
                reward = -self.trade_open_penalty
            ...
            if not self.position:
                ...
                # Small fixed penalty for opening a position (EXACTLY the same as for long)
                reward = -self.trade_open_penalty
        ...
        if self.current_step >= self.max_steps - 1:
            ...
            if self.position:
                ...
                if profit > 0:
                    reward = 0.05 * profit / self.initial_balance
                else:
                    reward = -0.05 * abs(profit) / self.initial_balance
            
            return self._next_observation(), reward, True, False, {}
        
        # Calculate net worth change for this step
        current_networth = self.get_networth()
        step_networth_change = current_networth - prev_networth
        
        # Add net worth change component to reward (scaled down)
        networth_reward = 0.5 * step_networth_change / self.initial_balance
        reward += networth_reward
        
        # Penalty for holding a position too long
        if self.position and self.trade_duration > 20:
            # Equal penalty for both long and short positions
